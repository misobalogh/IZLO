...
// Tato funkce by mela do formule pridat klauzule predstavujici podminku a) Každý předmět musí být zapsán alespoň jednou.
...
// Tato funkce by mela do formule pridat klauzule predstavujici podminku b) Každý předmět musí být zapsán nejvýše jednou.
// Predmety jsou reprezentovany cisly 0, 1, ..., num_of_subjects-1
// Semestry jsou reprezentovany cisly 0, 1, ..., num_of_semesters-1
...
// Tato funkce by mela do formule pridat klauzule predstavujici podminku c) Všechny prerekvizity musí být splněny.
...
void add_prerequisities_to_formula(CNF *formula, Prerequisity *prerequisities, unsigned num_of_prerequisities, unsigned num_of_subjects, unsigned num_of_semesters)
{
    ...
    for (unsigned i = 0; i < num_of_prerequisities; ++i)
    {
        ...
        {
            Clause *clause = create_new_clause(num_of_subjects, num_of_semesters);
            add_literal_to_clause(clause, false, prerequisities[i].later_subject, semester_j);
            for (unsigned semester_k = 0; semester_k < semester_j; ++semester_k)
            {
                add_literal_to_clause(clause, true, prerequisities[i].earlier_subject, semester_k);
            }
            add_clause_to_formula(clause, formula);
        }
    }
